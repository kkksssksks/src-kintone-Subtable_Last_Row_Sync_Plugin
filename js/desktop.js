(function(PLUGIN_ID) {
  ...
  const showProgressModal = () => {
    ...
    overlay.innerHTML = `<div style="background:#fff; padding:30px; border-radius:8px; text-align:center; min-width:400px; box-shadow:0 10px 25px rgba(0,0,0,0.2);"><h3 style="margin:0 0 10px; color:#2c3e50;">一括更新中 (Ver 1.1.2)</h3><p id="plugin-progress-text" style="color:#7f8c8d; font-size:14px; margin-bottom:5px;">準備中...</p><div style="width:100%; background:#ecf0f1; height:10px; border-radius:5px; overflow:hidden;"><div id="plugin-progress-bar" style="width:0%; height:100%; background:#3498db; transition:width 0.3s;"></div></div><p id="plugin-error-count" style="color:#e74c3c; font-size:12px; margin-top:10px; display:none;">エラースキップ: 0件</p></div>`;
  ...
  kintone.events.on('app.record.index.show', (event) => {
    ...
    btn.onclick = async () => {
      ...
      for (let i = 0; i < total; i += 100) {
        ...
        // 送信前にJSONとして不正な値(undefinedなど)を物理的に除去・変換
        ...
        try {
          ...
          // 100件まとめで失敗した場合のみ、1件ずつリトライしてエラー箇所を特定・スキップ
          for (const singleRec of safeChunk) {
            try {
              ...
              errorCount++; 
              processedCount++; 
              console.error(`ID ${singleRec.id} の更新失敗:`, singleErr);
            }
      ...
      closeProgressModal();
      alert('一括更新完了！' + (errorCount > 0 ? ` (${errorCount}件スキップしました。詳細はコンソールを確認してください)` : ''));
      location.reload();
    };
